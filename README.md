...
喫煙所検索サービスは地図を使用するため、ページ遷移の必要がなくなります。さらに今後の機能追加により画面の状態変化が多くなることを想定し、Reactを使用したシングルページアプリケーション（SPA）を採用しました。  
...
Railsとの親和性の高さからPostgreSQLを採用しました。
...
zodはAPIレスポンスの検証のために採用しました。TypeScriptの型はコンパイル時のもので、実際にAPIから返ってきた値が想定どおりの型かどうかは実行時に検証しない限り分からず、想定外の型のまま画面に表示せずエラーとして扱えるようにしています。
