...
import type {
  SmokingAreaDisplay,
  SmokingAreaSearchParams,
} from "./features/smokingAreas/types";
...
export const SmokingAreasMap = ({
  ...
  const { state: tobaccoTypesState, refetch: refetchTobaccoTypes } =
    useTobaccoTypes();

  const renderOverlay = () => {
    if (
      smokingAreasState.status === "error" ||
      tobaccoTypesState.status === "error"
    ) {
      return (
    ...
    if (
      smokingAreasState.status === "loading" ||
      tobaccoTypesState.status === "loading"
    ) {
  ...
  const getSelectedSmokingArea = (): SmokingAreaDisplay | null => {
    ...
    return (
      smokingAreasState.data.find(
        (smokingArea) => smokingArea.id === selectedId,
      ) ?? null
    );
  };
  ...
  const selectedTobaccoTypes =
    tobaccoTypesState.status === "success"
      ? formatSelectedTobaccoTypes(
          tobaccoTypesState.data,
          selectedTobaccoTypeIds,
        )
      : "";
    ...
    <div className="map-container" ref={mapContainerRef}>
      ...
      <APIProvider apiKey={apiKey} libraries={["marker"]}>
        ...
        >
          ...
          {smokingAreasState.status === "success" &&
            tobaccoTypesState.status === "success" &&
            smokingAreasState.data.map((smokingArea) => {
              const isSelected = selectedId === smokingArea.id;
              return (
                <AdvancedMarker
                  key={smokingArea.id}
                  position={{
                    lat: smokingArea.latitude,
                    lng: smokingArea.longitude,
                  }}
                  onClick={() =>
                    setSelectedId(isSelected ? null : smokingArea.id)
                  }
